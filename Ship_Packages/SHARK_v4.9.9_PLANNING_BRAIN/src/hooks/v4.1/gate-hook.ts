...
import { verifyRuntimeGradeEvidence } from '../../shared/verification-matrix.js';
import { 
  updateThoughtStream, updateCompactionSurvival, updateBuildStateOnTaskComplete,
  updateEvidenceState, updatePostCompactionPrompt, updateDebugLog,
  updateSoCPreservation, updateDecisionChain
...
export function createGateHook(
  ...
  return async (input, output) => {
        ...
        if (systemBrainRef) {
          try {
            ...
            if (violations.length > 0) {
              ...
              // HARD BLOCK: Semantic violations detected. Do NOT let this write through.
              throw new Error(`[ENFORCEMENT BLOCKED] Semantic violations detected (${violations.length}): ${violations.map(v => `[${v.id}] ${v.description}`).join('; ')}. File was NOT written.`);
            }
          } catch {
    ...
    if (currentGate === 'delivery' && !lastDeliveryBlocked) {
      ...
      try {
        ...
        if (planningBrain && planningBrain.enabled) {
          ...
          const allBehavioralPass = matrix.every(r => r.status === 'behavioral-pass');
          if (!allBehavioralPass) {
            const failing = matrix.filter(r => r.status !== 'behavioral-pass');
            logInfo(`Delivery blocked: verification matrix has ${failing.length} non-passing requirements: ${failing.map(r => `${r.id}:${r.status}`).join(', ')}`);
    ...
    try {
      ...
      console.error(`[GateHook-FallbackCtx] Context doc update failed for ${tool}: ${ctxErr instanceof Error ? ctxErr.message : String(ctxErr)}`);
    }
  };
}
...
function checkDeliveryGateBlocked(): boolean {
  // Runtime-Grade Firewall: Verify ALL mechanical evidence for runtime grade
  const evidence = verifyRuntimeGradeEvidence(process.cwd());
  if (!evidence.passed) {
    logInfo(`[RUNTIME-GRADE FIREWALL] Delivery blocked: missing evidence — ${evidence.missing.join(', ')}`);
    return true;
  }
  return false;
}
