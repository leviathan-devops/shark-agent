...
import {
  ...
  RUNTIME_GRADE_CLAIM_PATTERNS,
  RUNTIME_GRADE_EXEMPT_PATTERNS,
...
/**
 * Check if text contains a runtime grade claim that should be blocked.
 * Returns the firewall message if blocked, null if allowed/exempt.
 */
function detectRuntimeGradeClaim(text: string): string | null {
  // First check exemptions — questions, denials, audit requests are allowed
  for (const exemptPattern of RUNTIME_GRADE_EXEMPT_PATTERNS) {
    if (exemptPattern.test(text)) {
      return null; // Exempt — allow through
    }
  }

  // Check for runtime grade claim patterns
  for (const claimPattern of RUNTIME_GRADE_CLAIM_PATTERNS) {
    if (claimPattern.test(text)) {
      return `[RUNTIME-GRADE FIREWALL BLOCKED] You claimed "runtime grade" without mechanical verification.
...
function detectDerailment(text: string, sessionID: string): string | null {
  ...
  // RUNTIME-GRADE FIREWALL — highest priority, blocks false claims immediately
  const runtimeGradeBlock = detectRuntimeGradeClaim(text);
  if (runtimeGradeBlock) {
    incrementSlopScore(sessionID, 2); // Higher penalty for false runtime grade claims
    return runtimeGradeBlock;
  }
