...
export function createSharkGateTool(
  ...
  return tool({
    ...
    args: {
      action: z.enum(['evaluate', 'status', 'criteria', 'advance', 'verify']).describe('Action: evaluate a gate, get status, get criteria, advance to next gate, or verify runtime grade evidence'),
    ...
    execute: async (args, ctx) => {
      ...
      if (action === 'verify') {
        ...
        const result = {
          runtimeGradeVerified: evidence.passed,
