...
export function loadMatrix(basePath: string): VerificationMatrix {
  ...
  try {
    ...
    return safeParseJSON<VerificationMatrix>(raw) || createDefaultVerificationMatrix();
  } catch (err) {
...
export function updateRequirementStatus(
  ...
  return matrix.map((r: BehavioralRequirement) => r.id === id ? { ...r, status, lastChecked: Date.now() } : r);
}
