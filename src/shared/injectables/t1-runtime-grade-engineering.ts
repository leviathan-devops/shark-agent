...
// ═══════════════════════════════════════════════
// LANGUAGE GATE — prevents TS-specific detectors from firing on non-TS code
// ═══════════════════════════════════════════════

/**
 * Determine if a file is TypeScript/JavaScript based on its extension.
 *
 * Many T1 detectors check for TS/JS-specific syntax constructs:
 *   - `as` casts (P2) — Python uses `with...as`, `except...as`, `import...as`
 *   - `import { } from` (P1) — Python uses `from x import y`
 *   - `.then()`, `new Promise` (P9) — JS-only async patterns
 *   - function return types (P10) — TS-only syntax `(): Type`
 *
 * Applying these to non-TS files produces permanent false-positive blocks.
 * This gate must be called at the top of every TS-specific detector's detect().
 */
/** @deprecated Replaced by honesty-engine/ or candidate-generator.ts. Retained for backward compatibility. */
export function isTypeScriptFile(filePath: string): boolean {
  if (!filePath) return true; // Unknown file — run checks (safe default for TS projects)
  const ext = filePath.split('.').pop()?.toLowerCase();
  if (!ext) return true;
  // TypeScript / JavaScript extensions
  if (['ts', 'tsx', 'js', 'jsx', 'mts', 'mjs', 'cjs'].includes(ext)) return true;
  // Known non-TS/JS extensions — skip TS-specific checks entirely
  if (['py', 'rb', 'go', 'rs', 'java', 'c', 'cpp', 'cc', 'h', 'hpp',
       'sh', 'bash', 'zsh', 'yml', 'yaml', 'json', 'xml', 'html', 'htm',
       'css', 'scss', 'md', 'txt', 'toml', 'ini', 'cfg', 'sql', 'php',
       'swift', 'kt', 'scala', 'lua', 'pl', 'r', 'dart'].includes(ext)) {
    return false;
  }
  return true; // Unknown extension — default to checking (TS project assumption)
}
...
/**
 * FIXED (v5.1): Broadened regex to match typed catch parameters
 * like `catch (error: unknown) {` and `catch (e: Error) {`.
 * Previously only matched untyped `catch (e) {` and `catch {`.
 * Also correctly handles `catch { }` (no parameter) and
 * `catch (e) { }` (untyped parameter).
 */
function findCatchBlocks(code: string): Array<{ body: string; index: number }> {
  const results: Array<{ body: string; index: number }> = [];
  // Matches: catch {, catch(e){, catch (e) {, catch (error: unknown) {, catch(err: Error){
...
export const P1_DEFENSIVE_IMPORT: ViolationDetector = {
  ...
  detect(code: string, _ctx: CodeContext): boolean {
    // LANGUAGE GATE: `import { x } from 'path'` is TS/JS-specific syntax.
    // Python uses `from x import y`, Go uses `import "pkg"`, Rust uses `use pkg`.
    if (!isTypeScriptFile(_ctx.filePath)) return false;
...
export const P2_TYPE_CERTAINTY: ViolationDetector = {
  ...
  detect(code: string, ctx: CodeContext): boolean {
    // LANGUAGE GATE: `as` cast is TypeScript/JavaScript-specific.
    // Python uses `with...as`, `except...as`, `import...as` — NOT type casts.
    // Go uses `switch v := x.(type)` — NOT `as`.
    // Rust uses `use x as y` — NOT a type cast.
    // Without this gate, every Python file with `except Exception as e` gets blocked.
    if (!isTypeScriptFile(ctx.filePath)) return false;
...
export const P9_ASYNC_DISCIPLINE: ViolationDetector = {
  ...
  severity: 'critical',
  detect(code: string, _ctx: CodeContext): boolean {
    // LANGUAGE GATE: `.then()`, `new Promise`, `void async` are JS-specific.
    // Python uses `asyncio`, Go uses goroutines, Rust uses async/await differently.
    if (!isTypeScriptFile(_ctx.filePath)) return false;
...
export const P10_OUTPUT_CONTRACT: ViolationDetector = {
  ...
  severity: 'high',
  detect(code: string, _ctx: CodeContext): boolean {
    // LANGUAGE GATE: `function foo(): Type` return type annotation is TS-specific.
    // Python uses `-> Type`, Go uses named returns differently, Rust uses `-> Type` but different syntax.
    if (!isTypeScriptFile(_ctx.filePath)) return false;
