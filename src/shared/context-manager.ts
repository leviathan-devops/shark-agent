...
export function getContextDir(): string {
  if (!CONTEXT_DIR) {
    const base = process.env.SHARK_WORKSPACE || process.cwd();
    CONTEXT_DIR = process.env.SHARK_CONTEXT_DIR || path.join(base, PROJECT_TOKEN, 'CONTEXT_MANAGEMENT');
  }
  return CONTEXT_DIR;
}
...
export function initializeContextManager(workspaceBase?: string): string {
  ...
  // SHARK_WORKSPACE env var takes precedence (for container/host portability)
  // Default: active projects folder for this device
  // For public ships, export SHARK_WORKSPACE=/custom/path before starting opencode
