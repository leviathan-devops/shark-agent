...
function readJsonFile(filePath: string): Record<string, unknown> | null {
  try {
    ...
    console.warn('[audit-engine] readJsonFile failed:', readErr instanceof Error ? readErr.message : String(readErr));
    return null;
  }
}
...
export function scanForTheatricalCode(sourceDir: string): TheatricalCodeReport {
  ...
  if (fs.existsSync(fullSpectrumPath)) {
    try {
      const fullSpectrum = safeParseJSON(fs.readFileSync(fullSpectrumPath, 'utf-8')) as Record<string, unknown>;
      ...
      if (fullSpectrum.results) {
        for (const r of fullSpectrum.results as Record<string, unknown>[]) {
          if (!r.passed) {
            violations.push({
      ...
      console.warn('[audit-engine] FullSpectrum read failed:', fsErr instanceof Error ? fsErr.message : String(fsErr));
    }
  }
  ...
  // BUG-2 FIX: if sourceDir is a file (not a directory), scan it directly
  // instead of calling readdirSync which would throw ENOTDIR.
  try {
    const sourceStat = fs.statSync(sourceDir);
    if (sourceStat.isFile()) {
      scanSingleFile(sourceDir);
      return { clean: violations.length === 0, timestamp: new Date().toISOString(), violations };
    }
    if (!sourceStat.isDirectory()) {
      return { clean: false, timestamp: new Date().toISOString(), violations: [{ file: sourceDir, line: 0, pattern: 'not-a-directory', description: 'Source path is not a directory or file' }] };
    }
  } catch (statErr) {
    return { clean: false, timestamp: new Date().toISOString(), violations: [{ file: sourceDir, line: 0, pattern: 'stat-failed', description: `Cannot stat source: ${statErr instanceof Error ? statErr.message : String(statErr)}` }] };
  }

  function scanSingleFile(filePath: string): void {
    try {
      ...
      console.warn('[audit-engine] scanSingleFile read failed:', scanFileErr instanceof Error ? scanFileErr.message : String(scanFileErr));
    }
  }
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          if (dir.split(/[/\\]/).length - sourceDir.split(/[/\\]/).length < 5) {
            scanDir(fullPath);
          }
          ...
          try {
            ...
            console.warn('[audit-engine] scanDir file read failed:', scanFileErr instanceof Error ? scanFileErr.message : String(scanFileErr));
          }
        }
      }
    } catch (scanDirErr) {
      console.warn('[audit-engine] scanDir directory read failed:', scanDirErr instanceof Error ? scanDirErr.message : String(scanDirErr));
    }
  }
...
export function runFullAudit(sourceDir?: string): {
  ...
  try {
    ...
    console.warn('[audit-engine] evidence write failed:', writeErr instanceof Error ? writeErr.message : String(writeErr));
  }
