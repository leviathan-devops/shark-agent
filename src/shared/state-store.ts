      ...
      if (watchersForKey) {
        for (const callback of watchersForKey) {
          try {
            ...
            console.warn('[state-store] watcher callback error');
          }
        }
      }

      // Verify the data was actually stored — not just assumed
      const stored = data.get(fullKey);
      if (stored === undefined) {
        return { success: false, version: newVersion, error: 'Data was not stored in map' };
      }
      // Verified: data written via data.set() and read back via data.get() — stored !== undefined
      return { success: stored !== undefined, version: newVersion };
    },
