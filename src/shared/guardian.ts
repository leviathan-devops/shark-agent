...
import { isDangerousCommand as sharedIsDangerousCommand } from './danger-commands.js';
...
// Danger detection fully delegated to danger-commands.ts
...
export class Guardian {
  ...
  isDangerousCommand(command: string): boolean {
    return sharedIsDangerousCommand(command);
  }
  ...
  classifyZone(path: string): Zone {
    const expandedPath = path.replace(/^~/, process.env.HOME || os.homedir());
  ...
  private isFileWithinGracePeriod(filePath: string): boolean {
    try {
      ...
      // File deleted, moved, or permission denied — treat as outside grace period
      return false;
    }
  }
