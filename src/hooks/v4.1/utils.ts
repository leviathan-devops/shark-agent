 ...
 *
 * @identity-free — Pure utility functions. Identity checks are performed by the
 * calling hooks (tool-before, post-write-audit, etc.) before these helpers are
 * invoked. No agent-specific logic here; safe to call from any context.
 */
...
export function extractCommandFromArgs(args: unknown): string | null {
  if (!args) return null;
  const a = args as Record<string, unknown>;
  return (a.command as string) || (a.cmd as string) || (a.script as string) || null;
}
