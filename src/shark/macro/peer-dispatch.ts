...
import type { SharkMessenger as BrainMessenger } from '../../shared/messenger.js';
...
export class SharkPeerDispatch {
  ...
  /**
   * Handle gate failure — notify relevant brains
   */
  onGateFailed(gate: string, attempts: number): void {
    this.messenger.send({
      from: 'system-brain',
      to: 'reasoning-brain',
      type: 'alert',
      priority: 'high',
      payload: { gate, attempts, signal: 'gate-failed', timestamp: Date.now() },
      requiresAck: false,
    });
  }
