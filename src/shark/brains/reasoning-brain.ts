...
export interface ReasoningBrainState extends Omit<BrainState, 'state'> {
...
export function createReasoningBrain(config: ReasoningBrainConfig) {
  ...
  function updateState(updates: Partial<ReasoningBrainState['state']>): void {
    ...
    stateStore.write('thinking-state', 'shark-reasoning', next as BrainState);
  }
  ...
  function detectRuntimePatterns(sourceCode: string, context?: CodeContext): RuntimePattern[] {
    ...
    try {
      const candidates = generateCandidates(sourceCode, ctx?.filePath || '', { gate: currentGate });
      ...
      for (const candidate of candidates) {
        ...
        detected.push({
          id: `t1_${candidate.ruleId}`,
          category: categoryMap[candidate.category] || 'spec-fidelity',
      ...
      console.warn('[reasoning-brain] T1 candidate generation failed:', t1Err instanceof Error ? t1Err.message : String(t1Err));
    }
