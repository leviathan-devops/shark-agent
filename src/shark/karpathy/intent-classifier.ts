 ...
 *     //     confidence: 0.95, enforcement: 'CRITICAL', violation: '...', correction: '...' }
 *     ```
 *
 ...
 *     //     confidence: 0.9, enforcement: 'CRITICAL', violation: '...', correction: '...' }
 *     ```
 *
...
import { hasDestructiveArgs as sharedHasDestructiveArgs, evaluateBashCommand as sharedEvaluateBashCommand } from '../../shared/danger-commands.js';
 ...
 *
 * Non-destructive categories (QUERY, READ, TEST, EXPLORE, AUDIT, MANAGE) are
 * PASS in ALL gates — these tools can never modify anything.
 * Destructive categories (DESTRUCTIVE, CREATE, MODIFY, EXECUTE, DEPLOY) are
 * restricted in early/late gates as appropriate.
 */
const GATE_ENFORCEMENT_MATRIX: Record<IntentCategory, Record<GatePhase, EnforcementLevel>> = {
  DESTRUCTIVE: { PLAN: 'CRITICAL', BUILD: 'CRITICAL', VERIFY: 'CRITICAL', TEST: 'CRITICAL', AUDIT: 'CRITICAL', DELIVERY: 'CRITICAL' },
  CREATE:      { PLAN: 'PASS',  BUILD: 'PASS',  VERIFY: 'PASS',  TEST: 'PASS',  AUDIT: 'PASS',  DELIVERY: 'CRITICAL' },
  MODIFY:      { PLAN: 'CRITICAL', BUILD: 'PASS',  VERIFY: 'PASS',  TEST: 'PASS',  AUDIT: 'PASS',  DELIVERY: 'CRITICAL' },
  EXECUTE:     { PLAN: 'CRITICAL', BUILD: 'PASS',  VERIFY: 'PASS',  TEST: 'PASS',  AUDIT: 'PASS',  DELIVERY: 'CRITICAL' },
  QUERY:       { PLAN: 'PASS',  BUILD: 'PASS',  VERIFY: 'PASS',  TEST: 'PASS',  AUDIT: 'PASS',  DELIVERY: 'PASS'  },
  READ:        { PLAN: 'PASS',  BUILD: 'PASS',  VERIFY: 'PASS',  TEST: 'PASS',  AUDIT: 'PASS',  DELIVERY: 'PASS'  },
  TEST:        { PLAN: 'PASS',  BUILD: 'PASS',  VERIFY: 'PASS',  TEST: 'PASS',  AUDIT: 'PASS',  DELIVERY: 'PASS'  },
  EXPLORE:     { PLAN: 'PASS',  BUILD: 'PASS',  VERIFY: 'PASS',  TEST: 'PASS',  AUDIT: 'PASS',  DELIVERY: 'PASS'  },
  AUDIT:       { PLAN: 'PASS',  BUILD: 'PASS',  VERIFY: 'PASS',  TEST: 'PASS',  AUDIT: 'PASS',  DELIVERY: 'PASS'  },
  MANAGE:      { PLAN: 'PASS',  BUILD: 'PASS',  VERIFY: 'PASS',  TEST: 'PASS',  AUDIT: 'PASS',  DELIVERY: 'PASS'  },
  CLAIM:       { PLAN: 'MEDIUM',  BUILD: 'MEDIUM',  VERIFY: 'CRITICAL', TEST: 'CRITICAL', AUDIT: 'CRITICAL', DELIVERY: 'CRITICAL' },
  DEPLOY:      { PLAN: 'CRITICAL', BUILD: 'CRITICAL', VERIFY: 'CRITICAL', TEST: 'PASS',  AUDIT: 'CRITICAL', DELIVERY: 'PASS'  },
};

/**
 * Known tool name → IntentCategory mapping for tool call classification.
 *
 * All shark-* diagnostic/management tools are mapped to non-destructive
 * categories (QUERY, READ, TEST, EXPLORE, AUDIT, MANAGE) that PASS in ALL gates.
 */
const TOOL_INTENT_MAP: Record<string, { category: IntentCategory; action: string }> = {
  ...
  // ── Shark diagnostic tools — ALL non-destructive, PASS in ALL gates ──
  'shark-status':           { category: 'QUERY',  action: 'status' },
  'shark-gate':             { category: 'MANAGE',  action: 'gate' },
  'shark-evidence':         { category: 'QUERY',  action: 'evidence' },
  'shark-diagnose':         { category: 'QUERY',  action: 'diagnose' },
  'shark-health':           { category: 'QUERY',  action: 'health' },
  'shark-checkpoint':       { category: 'MANAGE',  action: 'checkpoint' },
  'shark-checkpoint-history': { category: 'QUERY',  action: 'checkpoint-history' },
  'shark-firewall-status':  { category: 'QUERY',  action: 'firewall-status' },
  'shark-firewall-audit':   { category: 'AUDIT',  action: 'firewall-audit' },
  'shark-hive-context':     { category: 'QUERY',  action: 'hive-context' },
  'shark-audit':            { category: 'AUDIT',  action: 'audit' },
  'shark-vision':           { category: 'EXPLORE', action: 'vision' },
  'shark-browser':          { category: 'EXPLORE', action: 'browser' },
  'shark-browser-test':     { category: 'TEST',   action: 'browser-test' },
  'shark-test-runner':      { category: 'TEST',   action: 'test-runner' },
  'shark-run-trident':      { category: 'AUDIT',  action: 'run-trident' },
  'shark-spawn-container':  { category: 'EXECUTE', action: 'spawn-container' },
};
...
export class IntentClassifier {
  ...
  classifyToolCall(toolName: string, args: Record<string, unknown>): IntentResult {
    // ─── READ_TOOLS GUARD ───────────────────────────────────────────────────
    // READ tools are NEVER destructive. Reading system files is always safe.
    // This guard must be BEFORE any other classification logic so that read
    // tools always pass through regardless of filePath or other args.
    const READ_TOOLS = new Set(['read', 'glob', 'grep', 'list', 'search', 'rg', 'find']);
    if (typeof toolName === 'string' && READ_TOOLS.has(toolName.toLowerCase().trim())) {
      return this.createEvidenceEntry(
        { intent: 'QUERY', action: toolName.toLowerCase().trim(), enforcement: 'PASS', confidence: 1.0, target: '' },
        `${toolName} ${JSON.stringify(args || {})}`, 'classifyToolCall',
      );
    }
    ...
    if (typeof toolName !== 'string') {
      ...
      return this.createEvidenceEntry(
        { ...FALLBACK_INTENT, confidence: 0.4, enforcement: 'MEDIUM', violation: `Invalid toolName type: ${typeof toolName}`, correction: 'Provide a valid string tool name' },
    ...
    try {
      ...
      if (normalizedTool.length === 0) {
        return this.createEvidenceEntry(
          { intent: 'EXECUTE', action: 'unknown', target: '', confidence: 0.4, enforcement: 'MEDIUM', violation: 'Empty tool name provided', correction: 'Provide a valid non-empty tool name' },
      ...
      if (!toolEntry) {
        return this.createEvidenceEntry(
          { intent: 'EXECUTE', action: normalizedTool, target: this.extractToolTarget(safeArgs), confidence: KNOWN_SHARK_TOOLS.has(normalizedTool) ? 0.9 : 0.4, enforcement: KNOWN_SHARK_TOOLS.has(normalizedTool) ? 'PASS' : 'MEDIUM', violation: KNOWN_SHARK_TOOLS.has(normalizedTool) ? '' : `Unknown tool "${toolName}" — treating as EXECUTE with warning`, correction: KNOWN_SHARK_TOOLS.has(normalizedTool) ? '' : 'Verify the tool is safe to use in the current context' },
      ...
      if (category === 'DESTRUCTIVE' || sharedHasDestructiveArgs(normalizedTool, safeArgs)) {
      ...
      if (normalizedTool === 'bash' && typeof safeArgs.command === 'string') {
        const bashEnforcement = sharedEvaluateBashCommand(safeArgs.command);
        if (bashEnforcement !== 'PASS') {
          enforcement = bashEnforcement;
          return this.createEvidenceEntry(
            { intent: category, action, target: safeArgs.command.substring(0, 80), confidence: 0.9, enforcement, violation: bashEnforcement === 'CRITICAL' ? 'Shell command blocked: contains dangerous pattern' : 'Shell command triggered warning', correction: 'Use targeted file operations instead of shell commands' },
      ...
      if (enforcement === 'CRITICAL' || enforcement === 'HIGH') {
      ...
      } else if (enforcement === 'MEDIUM' || enforcement === 'LOW') {
      ...
      return this.createEvidenceEntry(
        { ...FALLBACK_INTENT, confidence: 0.4, enforcement: 'MEDIUM', violation: `Internal classification error: ${errorMessage}`, correction: 'Retry the operation' },
        `${toolName} ${JSON.stringify(safeArgs)}`, 'classifyToolCall',
      );
    }
  }
  ...
  setGate(gate: GatePhase): void {
    ...
    const upperGate = gate.toUpperCase() as GatePhase;
    if (!VALID_GATES.includes(upperGate)) {
      return; // silently ignore — case conversion handles it
    }
    this.currentGate = upperGate;
  }
