...
export interface SemanticFrame {
  ...
  allowedGates?: string[];
  description: string;
  /** 0.0-1.0, updated by StatisticalNLP over time (W1.4) */
  statisticalWeight?: number;
  /** How many times this frame matched (W1.4) */
  frequency?: number;
  /** Timestamp of last match (W1.4) */
  lastMatchedAt?: number;
  /** True if generated by NLP, not hand-written (W1.4) */
...
export class VerbFrameLexicon {
  ...
  private initializeFrames(): void {
    const allFrames: SemanticFrame[] = [
      ...
      {
        verbs: ['scaffold', 'init', 'bootstrap', 'new'],
        category: 'CREATE',
        dangerLevel: 'LOW',
        allowedGates: ['PLAN', 'BUILD', 'TEST', 'VERIFY'],
      ...
      // ── DIAGNOSTIC/MANAGEMENT (non-destructive, allowed in ALL gates) ──
      {
        verbs: ['status', 'diagnose', 'health', 'evaluate', 'criteria', 'check', 'inspect', 'list', 'show'],
        category: 'QUERY',
        dangerLevel: 'NONE',
        allowedGates: ['PLAN', 'BUILD', 'TEST', 'VERIFY', 'AUDIT', 'DELIVERY'],
        description: 'Diagnostic and status tools — read-only, never destructive',
      },
      {
        verbs: ['gate', 'advance', 'transition', 'checkpoint'],
        category: 'MANAGE',
        dangerLevel: 'LOW',
        allowedGates: ['PLAN', 'BUILD', 'TEST', 'VERIFY', 'AUDIT', 'DELIVERY'],
        description: 'Gate management and checkpoint operations',
      },
      {
        verbs: ['explore', 'browse', 'navigate', 'vision'],
        category: 'EXPLORE',
        dangerLevel: 'NONE',
        allowedGates: ['PLAN', 'BUILD', 'TEST', 'VERIFY', 'AUDIT', 'DELIVERY'],
        description: 'Exploration and browsing — read-only observation',
      },
      {
        verbs: [
          'shark-status', 'shark-gate', 'shark-evidence', 'shark-diagnose', 'shark-health',
          'shark-checkpoint', 'shark-checkpoint-history', 'shark-firewall-status',
          'shark-firewall-audit', 'shark-hive-context', 'shark-audit', 'shark-vision',
          'shark-browser', 'shark-browser-test', 'shark-run-trident',
        ],
        category: 'QUERY',
        dangerLevel: 'NONE',
        allowedGates: ['PLAN', 'BUILD', 'TEST', 'VERIFY', 'AUDIT', 'DELIVERY'],
        description: 'Shark diagnostic/management tools — registered by tool name, always allowed in ALL gates',
      },
      // ── TEST RUNNER (restricted — not allowed in PLAN gate) ──────────────
      {
        verbs: ['shark-test-runner'],
        category: 'TEST',
        dangerLevel: 'LOW',
        allowedGates: ['BUILD', 'TEST', 'VERIFY', 'AUDIT'],
        description: 'Test runner — restricted from PLAN gate to enforce test-before-execute discipline',
      },
    ];
  ...
  getAllVerbFrames(): VerbFrame[] {
    try {
      return Array.from(this.verbFrames.values()).map((vf: VerbFrame) => ({
        ...vf,
        slots: vf.slots.map((s: FrameSlot) => ({ ...s })),
      }));
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  isAllowedInGate(verb: unknown, gate: unknown): boolean {
    try {
      ...
      // shark-* tools are generally diagnostic/management — allowed in ALL gates by default.
      // However, specific tools (e.g. shark-test-runner) may have frame-level gate restrictions
      // in their allowedGates array. Check the frame FIRST to respect those restrictions.
      if (validatedVerb.startsWith('shark-')) {
        const frame = this.frames.get(validatedVerb);
        if (frame && frame.allowedGates) {
          if (frame.allowedGates.length === 0) return false;
          return frame.allowedGates.includes(validatedGate);
        }
        // No frame or no gate restrictions — diagnostic tools allowed in all gates
        return true;
      }

      const frame = this.frames.get(validatedVerb);
      if (!frame) {
        // Check if tool name starts with a known verb prefix
        const baseName = validatedVerb.replace(/^mcp_/, '').replace(/_/g, '');
        let matchedFrame: SemanticFrame | undefined;
        for (const [, f] of this.frames) {
          if (f.verbs.some((v: string) => baseName.startsWith(v))) {
            matchedFrame = f;
            break;
          }
        }
        if (matchedFrame) return (matchedFrame.allowedGates || []).includes(validatedGate);
        // Truly unknown tool — DENY in late gates, ALLOW in early gates
        return validatedGate === 'PLAN' || validatedGate === 'BUILD';
      }
  ...
  /**
   * Update statistical weights from NLP analysis (W1.4).
   * Call this after StatisticalNLP produces FrameCandidates.
   *
   * Uses exponential moving average: prevWeight * 0.7 + confidence * 0.3
   * to smoothly converge on observed match confidence over time.
   */
  updateStatisticalWeight(verb: string, confidence: number): void {
    const frame = this.frames.get(verb);
    if (!frame) return;
    frame.frequency = (frame.frequency ?? 0) + 1;
    frame.lastMatchedAt = Date.now();
    // Exponential moving average for statistical weight
    const prevWeight = frame.statisticalWeight ?? 0.5;
    frame.statisticalWeight = prevWeight * 0.7 + confidence * 0.3;
  }
