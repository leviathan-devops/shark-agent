...
async function updateBuildJourney(
  ...
  try {
    ...
    console.warn('[checkpoint] journey file read failed, creating new');
...
export async function createPhaseSnapshot(
  ...
  const gate = (completedGate || gm.getCurrentGate()) as GateName;
  ...
  for (const subdir of ['src', 'dist', 'compaction_survival']) {
    ...
    try {
      ...
      console.warn('[checkpoint] copyDir failed (directory may not exist yet)');
    }
  }
...
export function createCheckpointTool(
  ...
  return tool({
    ...
    execute: async (args) => {
      ...
      // Wire RecoveryTracker warhead
      try {
        const recoveryW = getWarhead('recovery-tracker');
        if (recoveryW && typeof recoveryW === 'object' && 'setCheckpoint' in recoveryW) {
          const gmState = _gateManager.getState() as { currentGate: string };
          (recoveryW as { setCheckpoint: (t: string, d: string) => void }).setCheckpoint(
            new Date().toISOString(),
            `Checkpoint: ${message || 'manual checkpoint'} at gate ${gmState.currentGate || 'unknown'}`
          );
        }
      } catch (err) {
        console.error('[Checkpoint] Operation failed: ' + (err instanceof Error ? err.message : String(err)));
        // Non-fatal — recovery tracking is best-effort
      }
