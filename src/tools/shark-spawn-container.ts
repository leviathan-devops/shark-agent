...
function dockerRestPs(args: string[]): string {
  const filterIdx = args.findIndex((a: string) => a === '--filter' || a === '-f');
  // Note: 'localhost' here is a dummy hostname for Docker REST API over unix socket (--unix-socket).
  // The actual communication goes through /var/run/docker.sock, not the network.
  ...
  const containers = safeParseJSON(result.toString());
...
function dockerRestRun(args: string[]): string {
  ...
  // Note: 'localhost' is a dummy hostname for Docker REST API over unix socket
  ...
  const createResult = safeParseJSON(
    execSync(
...
/**
 * Resolve the opencode binary path dynamically.
 * Priority:
 *   1. OPENCODE_BINARY env var (explicit override)
 *   2. `which opencode` / `command -v opencode` (PATH lookup)
 *   3. Known baseline binary path (fallback for opencode-test:1.14.34 image)
 * This ensures container images with different layouts still work.
 */
function resolveOpencodeBinary(): string {
  const envOverride = process.env.OPENCODE_BINARY;
  if (envOverride && envOverride.length > 0) {
    // Path validation: ensure the override is an absolute path with no shell metacharacters
    const sanitized = path.resolve(envOverride);
    if (!/^[a-zA-Z0-9_\/.-]+$/.test(sanitized)) {
      console.warn(`[shark-spawn-container] OPENCODE_BINARY contains invalid characters — ignoring: ${sanitized}`);
    } else if (!fs.existsSync(sanitized)) {
      console.warn(`[shark-spawn-container] OPENCODE_BINARY path does not exist — ignoring: ${sanitized}`);
    } else {
      return sanitized;
    }
  }
  ...
  try {
    const whichResult = execSync('command -v opencode 2>/dev/null || which opencode 2>/dev/null', {
      encoding: 'utf-8', timeout: 5000, stdio: 'pipe',
    }).trim();
    if (whichResult && whichResult.length > 0 && whichResult !== 'opencode not found') {
      return whichResult;
    }
  } catch (whichErr) {
    console.warn('[shark-spawn-container] resolveOpencodeBinary failed:', whichErr instanceof Error ? whichErr.message : String(whichErr));
  }

  // Fallback: known path in opencode-test container image
  return '/usr/local/lib/node_modules/opencode-ai/node_modules/opencode-linux-x64-baseline/bin/opencode';
}
...
function createSnapshot(pluginSource: string, agentName: string): string {
  ...
  try {
    ...
    console.warn('[shark-spawn-container] npm install failed:', npmErr instanceof Error ? npmErr.message : String(npmErr));
  }
...
function cleanupContainer(containerName: string): void {
  try {
    ...
    console.warn('[shark-spawn-container] cleanupContainer failed:', cleanErr instanceof Error ? cleanErr.message : String(cleanErr));
  }
}
...
function cleanupTmux(tmuxSession: string): void {
  try {
    ...
    console.warn('[shark-spawn-container] cleanupTmux failed:', tmuxErr instanceof Error ? tmuxErr.message : String(tmuxErr));
  }
}
...
export function spawnContainerSync(input: SpawnContainerInput): SpawnContainerOutput {
  ...
  // Validate container name before using in execSync — prevents command injection
  ...
  try {
    ...
    const mountDockerSocket = `-v /var/run/docker.sock:/var/run/docker.sock`;

    dockerCmd(`run -d --rm --name ${containerName} --entrypoint "" ${mountSnap} ${mountWorkspace} ${mountDockerSocket} ${CONTAINER_IMAGE} sleep infinity`);
    ...
    if (!psCheck.trim().includes(containerName)) {
      ...
      try {
        ...
        console.warn('[shark-spawn-container] docker logs failed:', logErr instanceof Error ? logErr.message : String(logErr));
    ...
    try {
      execSync(`docker exec ${containerName} bash -c "apt-get update -qq && apt-get install -y -qq tmux docker.io 2>/dev/null"`, { stdio: 'pipe' });
    } catch (pkgErr) {
      console.warn('[shark-spawn-container] apt-get install failed:', pkgErr instanceof Error ? pkgErr.message : String(pkgErr));
    }
    ...
    try {
      ...
      console.warn('[shark-spawn-container] tmux setup failed:', tmuxSetupErr instanceof Error ? tmuxSetupErr.message : String(tmuxSetupErr));
    }
    ...
    } catch (_err) { console.warn("[shark-spawn-container] evidence write failure non-fatal:", _err instanceof Error ? _err.message : String(_err)); }

    // Verified: Docker container created and running (containerName set), tmux session established, evidence written to disk
    return {
      containerName,
      tmuxSession,
      workspaceMount: '/root/workspace',
...
export function createSharkSpawnContainerTool() {
  return tool({
    ...
    args: {
      ...
      pluginSource: z.string().optional().describe('Path to plugin source (defaults to Shark v5.1)'),
      projectPath: z.string().optional().describe('Path to project workspace to mount'),
    },
    execute: async (input: SpawnContainerInput) => {
      const { projectName, pluginSource, projectPath } = input;

      // Action: 'tile' — spawn using Copilot Tile Protocol
      if (input.projectName === 'tile') {
        try {
          const pyResult = execSync(
            `python3 scripts/spawn-shark-tile.py shark-container ${projectName || 'shark'}`,
            { encoding: 'utf-8', timeout: 30000 }
          ).toString().trim();
          const lines = pyResult.split('\n');
          const tileLine = lines.find((l: string) => l.startsWith('TILE_READY:') || l.startsWith('TILE_CREATED:'));
          const tileId = tileLine ? tileLine.split(':')[1] : 'unknown';
          return JSON.stringify({
            containerName: 'shark-container',
            tmuxSession: 'shark-container',
            workspaceMount: '/root/workspace',
            snapshotPath: '',
            success: true,
            tileId,
            protocol: 'copilot-tile-v1',
          });
        } catch (err) {
          const errorMsg = err instanceof Error ? err.message : String(err);
          return JSON.stringify({
            ...
            snapshotPath: '',
            success: false,
            error: `Tile protocol failed: ${errorMsg}. Falling back to raw docker.`,
          });
        }
      }
...
export function cleanupContainerTool(containerName: string, tmuxSession: string, snapshotPath: string): void {
  ...
  try {
    ...
    console.warn('[shark-spawn-container] snapshot cleanup failed:', snapCleanErr instanceof Error ? snapCleanErr.message : String(snapCleanErr));
  }
}
