...
// TODO: Make VLM endpoint configurable for remote deployment via VLM_HOST / VLM_PORT env vars
const VLM_HOST = process.env.VLM_HOST || '127.0.0.1';
const VLM_PORT = parseInt(process.env.VLM_PORT || '8082', 10);
const VLM_ENDPOINT = `http://${VLM_HOST}:${VLM_PORT}/v1/chat/completions`;
...
function getContainerImage(containerName: string): string {
  try {
    return execSync(
      ...
      console.warn('[shark-browser-test] getContainerImage failed');
      return '';
    }
  }
...
function findTestContainer(): string | null {
  try {
    ...
    console.warn('[shark-browser-test] findTestContainer failed');
    return null;
  }
}
...
export function createSharkBrowserTestTool() {
  return tool({
    ...
    execute: async (args: Record<string, unknown>) => {
      const { file, port, container: explicitContainer } = args as { file: string; port?: number; container?: string; action: string };
      ...
      // Validate paths to prevent traversal attacks before using in execSync/writeFileSync/mkdirSync
      resolveSecurePath(fileDir, process.cwd());
      resolveSecurePath(browserTestDir, process.cwd());
      ...
      } catch (_err) { console.warn("[shark-browser-test] non-fatal:", _err instanceof Error ? _err.message : String(_err)); }
      ...
      try {
        // TODO: Make HTTP server bind address configurable for remote deployment
      ...
      try {
        // TODO: Make browser endpoint configurable for remote deployment
      ...
      try {
        ...
        const parsed = safeParseJSON(errors);
      ...
      try {
        ...
        " 2>&1`);
        const domData = safeParseJSON(domResult);
        result.domCheck = domData as Record<string, unknown>;
      } catch (_err) { console.warn("[shark-browser-test] non-fatal:", _err instanceof Error ? _err.message : String(_err)); }
      ...
      try {
        ...
        if (screenshotB64 && screenshotB64.length > 100) {
          ...
          try {
            ...
            const vlmData = safeParseJSON(vlmResult);
            const vlmParsed = vlmData as { choices?: Array<{ message?: { content?: string } }> } | null;
            result.visualAnalysis = vlmParsed?.choices?.[0]?.message?.content || '';
          } catch (_err) { console.warn("[shark-browser-test] non-fatal:", _err instanceof Error ? _err.message : String(_err)); }
        }
      } catch (_err) { console.warn("[shark-browser-test] non-fatal:", _err instanceof Error ? _err.message : String(_err)); }
