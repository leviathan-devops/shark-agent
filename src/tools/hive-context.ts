 ...
 *   4. ~/.local/share/opencode/hive-mind
 *
...
function globDirs(root: string, pattern: RegExp): string[] {
  try {
    ...
    console.warn('[hive-context] globDirs failed:', globDirErr instanceof Error ? globDirErr.message : String(globDirErr));
    return [];
  }
}
...
function globFiles(root: string, pattern: RegExp): string[] {
  try {
    ...
    console.warn('[hive-context] globFiles failed:', globFileErr instanceof Error ? globFileErr.message : String(globFileErr));
    return [];
  }
}
...
function findFiles(
  ...
  function walk(dir: string, depth: number) {
    ...
    try {
      ...
      console.warn('[hive-context] findFiles walk failed:', walkErr instanceof Error ? walkErr.message : String(walkErr));
      return;
    }
...
function readFileSafe(filePath: string): string | null {
  try {
    ...
    console.warn('[hive-context] readFileSafe failed:', readErr instanceof Error ? readErr.message : String(readErr));
    return null;
  }
}
...
function readDirMarkdown(dirPath: string): string[] {
  ...
  try {
    ...
    console.warn('[hive-context] readDirMarkdown failed:', dirErr instanceof Error ? dirErr.message : String(dirErr));
  }
  return parts;
}
...
function searchHiveMind(root: string, query: string): Array<{ file: string; line: number; snippet: string }> {
  ...
  function walk(dir: string, depth: number) {
    ...
    try {
      ...
      console.warn('[hive-context] searchHiveMind walk failed:', searchWalkErr instanceof Error ? searchWalkErr.message : String(searchWalkErr));
      return;
    }
    for (const entry of entries) {
      const full = path.join(dir, entry.name);
      if (entry.isDirectory() && !entry.name.startsWith('.')) {
        ...
        try {
          ...
          console.warn('[hive-context] searchHiveMind file read failed:', searchFileErr instanceof Error ? searchFileErr.message : String(searchFileErr));
        }
      }
    }
  }
...
function listAvailableTopics(root: string): string[] {
  ...
  function walk(dir: string, depth: number) {
    ...
    try {
      ...
      console.warn('[hive-context] listAvailableTopics walk failed:', topicWalkErr instanceof Error ? topicWalkErr.message : String(topicWalkErr));
      return;
    }
    for (const entry of entries) {
      const full = path.join(dir, entry.name);
  ...
  try {
    ...
    console.warn('[hive-context] listAvailableTopics top-level failed:', listErr instanceof Error ? listErr.message : String(listErr));
  }
...
function countMdFiles(dir: string): number {
  try {
    ...
    console.warn('[hive-context] countMdFiles failed:', countErr instanceof Error ? countErr.message : String(countErr));
    return 0;
  }
}
...
export function createHiveContextTool() {
  return tool({
    ...
    execute: async (args: Record<string, unknown>, _ctx: Record<string, unknown>): Promise<string> => {
      const action = String((args && args.action) || 'read');
      const topic = (args && args.topic) ? String(args.topic) : undefined;
      const query = (args && args.query) ? String(args.query) : undefined;
